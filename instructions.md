...
| Stage 2 | `positive_ctxs[0]` | `hard_negative_ctxs` 中采样最多7个（不足则按实际数量） | L_InfoNCE + w × L_dis | 直接使用现有字段 |
...
Stage 2 (BM25 Hard Neg, 8 epochs)
  ...
  - 每个query采样最多7个BM25难负例参与训练（不足则按实际数量）
  - 损失函数: L_InfoNCE + w × L_dis（w恒定）
  - 纯GPU训练
...
输入：nq-train.json, 58880条
每条取：question + positive_ctxs[0] + hard_negative_ctxs中采样最多7个
负例组成：最多7个BM25难负例 + batch内其他query的positive（混合）
损失：L_InfoNCE + 0.6 × L_dis
...
每条取：question + positive + mined_hard_negatives中采样最多7个
损失：L_InfoNCE + 0.6 × L_dis
